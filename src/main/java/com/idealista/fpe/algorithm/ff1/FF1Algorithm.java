...
class FF1Algorithm {
    ...
    private static final int NUMBER_OF_ROUNDS = 10; //ÂÖÊýÎª10
    ...
    static int[] encrypt(int[] plainText, Integer radix, byte[] tweak, PseudoRandomFunction pseudoRandomFunction) {
        ...
        int leftSideLength = target.leftSideLength();   // u
        int rightSideLength = target.rightSideLength(); //v
        int lengthOfLeftAfterEncoded = (int) ceil(ceil(rightSideLength * log(radix)) / 8.0);  //b
        int paddingToEnsureFeistelOutputIsBigger = (int) (4 * ceil(lengthOfLeftAfterEncoded / 4.0) + 4); //d
        ByteString padding = generateInitialPadding(radix, target.length(), tweak.length, leftSideLength); //P

        int[] left = target.left(); //A
        int[] right = target.right();  //B
        for (int round=0; round<NUMBER_OF_ROUNDS; round++) {
            BigInteger roundNumeral = roundNumeral(num(right, radix), tweak, padding, pseudoRandomFunction, lengthOfLeftAfterEncoded, paddingToEnsureFeistelOutputIsBigger, round); //S
            int partialLength = round % 2 == 0 ? leftSideLength : rightSideLength; // be sure of m
